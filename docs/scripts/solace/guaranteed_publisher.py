...
TOPIC_PREFIX = os.environ["SOLACE_TOPIC"] = "aims/import/article"

broker_props = {
    "solace.messaging.transport.host": os.environ.get('SOLACE_HOST') or "20.113.12.89",
...
lidl_article = load(open("lidl-article.json"))
message_body = lidl_article.copy()
...
try:
    while True:
        message_body["id"]=str(count)
        outbound_msg = outbound_msg_builder \
                    .with_application_message_id(f'solace-test {count}')\
                    .build(message_body)
